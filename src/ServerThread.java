	...
	public void run() {
		...
		try {
			...
			if (tag.contentEquals("server")) {
				...
				sendServerRequest(com); // Send a request to every server. Wait
										// for reply from all other servers

				Server.checkQueue(com); // Wait for own request to be at the
										// head of the queue

				String response = processCommand(st, tag); // Enter critical
															// section
				sendClientResponse(response, tcpOutput);

				Server.dequeue(); // Remove request from own queue

				notifyServer(); // Notify other servers to remove request from
								// their queues
	...
	private String processCommand(Scanner st, String tag) {
		...
		if (tag.contentEquals("purchase")) {
			...
			if ((p = inv.getProduct(productName)) == null) {
				...
				response = "You order has been placed, " + orderId + " " + username + " " + productName + " "
						+ quantity;
